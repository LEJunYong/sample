...
public class UploadController {
	...
	public ResponseEntity<Resource> downloadFile(String fileName){
		...
		// 해더를 통해서 이 파일을 다운로드 할수 있다고 브라우저에게 알려줘야한다.
		// 이 경로를 통해 요청하게 되면 브라우저는 자동으로 해당 파일을 다운로드하도록 하게 만듬.
	...
	// 우리가 생각해야 할점 : 섬네일 이미지의 이름 '파일경로' + 's_' 'uuid' + '파일의 원래이름'
	// 그리고 한글 처리까지
	...
	public ResponseEntity<byte[]> getFile(String fileName) {
		...
		try {
			...
			// probeContentType type : 해당 파일의 Content type 조사
	...
	@PostMapping("/uploadFormAction")
	   public void uploadFormAction(MultipartFile[] uploadFile , Model model) {
	      ...
	      for(MultipartFile file : uploadFile) {
	         ...
	         //이것이랑 헷갈리지말것 getName: input 태그의 name 속성 값 반환
	...
	private boolean checkImageType(File file) {
		// 파라미터로 받아온 file이 이미지 파일이면 return turn;
		// 파리미터로 받아온 file이 이미지가 아니면 return false;
