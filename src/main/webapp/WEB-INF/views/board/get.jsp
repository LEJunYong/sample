	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				aria-hidden="true">&times;</button><!-- x  &nbp-->
			...
			<div class="modal-body">
				<div class="form-group">
					<label>Replyer</label>
					<input class="form-control" name="replyer" value="reply">
		</div>
	</div>
	<!-- 버튼 모음 (수정, 삭제 , 등록, 닫기....-->
...
<script type="text/javascript">
   var replyService = (function() {
	...
	function displayTime(timeValue) {
		...
		var str = ""; //리턴할 날짜(오늘댓글이면  시간, 어제 이전에 댓글이면 날짜)
 ...
 */
 $(document).ready(function () {
	 ...
	 //요청이 성공했으면 그떄 댓글을 만든다 ==> callback 함수에서 처리하면 된다.
	 showList(1);
	 ...
	 function showList(page) {
		 ...
		 replyService.getList({
			 ...
			 if(page == -1){
				 //댓글 개수가 7개 일때 페이지 몇개? (1페이지에 10개라고 하면 ) 페이지 개수 1개
	 ...
	 function showReplyPage(replyCnt) {
		...
		let prev = startNum != 1;
		 ...
		 //다음 페이지가 존해하면 (10개 단위) next 버튼 활성화
		 if(next) {
			 pageHtml += "<li class='page-item'>";
	 ...
	 // 앞으로 보낼 모든 ajax 요청의 헤데에 csrf 토근 정보를 담아서 보낼수 있도록 설정
     ...
     $("#modalRegisterBtn").on("click" , function (e) {
		 ...
		 // 게시글 번호 bno 가져와서 reply 객체 만든뒤에 닷글 달기 기능 실행
		 var reply = {
	...
	// 동적으로 생성될 요소의 부모또는 형제 요소에 다가 달아준 다음
	// 나중에 이벤트의 대상을 변경해준다.
	...
	$("#modalModBtn").on("click" , function(e){
		...
		replyPageFooter.on("click" , "li a", function(e) {
			...
			// 이동할 페이지 번호
			// href 속성에 페이지 번호를 저장 해놨으르로 꺼내서 쓰자 .
